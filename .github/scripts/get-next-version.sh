...
# Redirect cd command output to stderr
cd packages/$PACKAGE_NAME >&2
...
if ! git tag -l "${PACKAGE_NAME}/v*" | grep -q .; then
  ...
  # Redirect semantic-release output to stderr
  VERSION=$(npx semantic-release \
    --dry-run \
    --no-ci \
    --tag-format "${PACKAGE_NAME}/v\${version}" \
    --plugins "@semantic-release/commit-analyzer,@semantic-release/release-notes-generator" \
    --debug 2>&1 >&2 | grep -oP "(?:version: '|The next release version is )\K[0-9]+\.[0-9]+\.[0-9]+" || true)
...
# Output only the version to stdout, everything else to stderr
printf "v%s" "${VERSION}" >&1
